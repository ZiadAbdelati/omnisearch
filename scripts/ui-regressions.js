...
(async () => {
  ...
  assert.match(html, /href="\/styles\.css\?v=45"/, 'HTML should bust cached CSS after mobile API key list redesign');
  assert.match(html, /src="\/app\.js\?v=33"/, 'HTML should bust cached app JS after mobile API key list redesign');
  ...
  assert.match(appJs, /class="mobile-account-list"/, 'mobile account cards should use settings-style label/value rows instead of stat tiles');
  assert.match(appJs, /class="mobile-account-row"/, 'mobile account configuration should render as compact list rows');
  assert.doesNotMatch(appJs, /class="mobile-fields">\s*<div class="mobile-field"><span>Enabled<\/span>/, 'mobile account cards must not render desktop-style enabled stat tiles');
  assert.match(appJs, /class="mobile-account-actions actions"/, 'mobile account actions should use their dedicated compact layout');
  assert.match(appJs, /class="danger mobile-account-delete"/, 'mobile delete action should have its own full-width safe target');
  assert.match(fs.readFileSync('public/styles.css', 'utf8'), /\.mobile-account-list\s*\{[^}]*border-top/, 'mobile account list should use dividers rather than nested boxes');
  assert.match(fs.readFileSync('public/styles.css', 'utf8'), /\.mobile-account-row\s*\{[^}]*justify-content:\s*space-between/, 'mobile account rows should align labels and values naturally');
  assert.match(fs.readFileSync('public/styles.css', 'utf8'), /\.mobile-account-delete\s*\{[^}]*grid-column:\s*1\s*\/\s*-1/, 'mobile delete action should be separated across the full card width');
  assert.match(fs.readFileSync('public/styles.css', 'utf8'), /\.mobile-account-actions\.actions\s*\{[^}]*display:\s*grid/, 'mobile account actions must explicitly override desktop flex actions');
  assert.match(fs.readFileSync('public/styles.css', 'utf8'), /\.mobile-account-actions \.mobile-account-delete\s*\{[^}]*grid-column:\s*1\s*\/\s*-1/, 'mobile delete action must span the action grid despite shared desktop action styles');
  assert.match(fs.readFileSync('public/styles.css', 'utf8'), /\.account-mobile-card\s*\{[^}]*min-width:\s*0/, 'mobile account cards must not expand past their page rail');
  assert.match(fs.readFileSync('public/styles.css', 'utf8'), /\.mobile-account-row \.mobile-check\s*\{[^}]*flex-direction:\s*row/, 'mobile account status controls should stay horizontally aligned inside their list row');
  assert.match(appJs, /class="mobile-key-list"/, 'mobile API key cards should use settings-style label/value rows');
  assert.match(appJs, /class="mobile-key-row"/, 'mobile API key details should render as compact list rows');
  assert.doesNotMatch(appJs, /class="mobile-fields">\s*<div class="mobile-field mobile-field-wide"><span>Key<\/span>/, 'mobile API key cards must not render desktop-style stat tiles');
  assert.match(appJs, /class="mobile-key-actions actions"/, 'mobile API key actions should use their dedicated compact layout');
  assert.match(appJs, /class="danger mobile-key-delete"/, 'mobile API key delete action should have a separate safe target');
  assert.match(fs.readFileSync('public/styles.css', 'utf8'), /\.mobile-key-list\s*\{[^}]*border-top/, 'mobile API key list should use dividers rather than nested boxes');
  assert.match(fs.readFileSync('public/styles.css', 'utf8'), /\.mobile-key-row\s*\{[^}]*justify-content:\s*space-between/, 'mobile API key rows should align labels and values naturally');
  assert.match(fs.readFileSync('public/styles.css', 'utf8'), /\.mobile-key-actions\.actions\s*\{[^}]*display:\s*grid/, 'mobile API key actions must explicitly override desktop flex actions');
  assert.match(fs.readFileSync('public/styles.css', 'utf8'), /\.mobile-key-actions \.mobile-key-delete\s*\{[^}]*grid-column:\s*1\s*\/\s*-1/, 'mobile API key delete action should span the action grid');
  ...
  assert.match(html, /<script src="\/app\.js\?v=33"><\/script>/, 'HTML should bust cached app JS after mobile API key list redesign');
  ...
  assert.match(html, /<script src="\/app\.js\?v=33"><\/script>/, 'HTML should keep the mobile account renderer cache-busted alongside the API key renderer');
